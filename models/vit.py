...
def vit(image_size, patch_size, num_classes, channels):
    model = ViT(
        ...
        dropout=0.1,
        emb_dropout=0.1,
        channels=channels  # 传递 channels 参数
    )
    return model


if __name__ == '__main__':
    img = torch.randn(16, 1, 64, 64)
    model = vit(image_size=64, patch_size=4, num_classes=38, channels=1)
    preds = model(img)
    print(preds.shape)
    pass
