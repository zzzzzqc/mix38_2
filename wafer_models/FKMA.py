...
class DWTFunction(Function):
    @staticmethod
    def forward(ctx, x, wavelet='sym2'):
        # 确保输入是4D张量 [batch, channel, height, width]
        assert x.dim() == 4, "Input must be 4D tensor"

        # 获取小波滤波器
        w = pywt.Wavelet(wavelet)
        dec_hi = torch.tensor(w.dec_hi[::-1], dtype=x.dtype, device=x.device)
        dec_lo = torch.tensor(w.dec_lo[::-1], dtype=x.dtype, device=x.device)
        ...
        # 扩展滤波器以适应输入通道
        filters = filters.unsqueeze(1).repeat(1, x.size(1), 1, 1, 1)
        filters = filters.to(x.device)

        # 应用卷积进行小波变换
        stride = 2
        padding = (filters.size(-1) // 2)
        y = F.conv2d(x, filters, stride=stride, padding=padding, groups=x.size(1))

        # 分离四个子带: AA, AD, DA, DD
        bs, c, h, w = y.shape
        y = y.view(bs, 4, c, h, w)
        aa, ad, da, dd = y[:, 0], y[:, 1], y[:, 2], y[:, 3]
    ...
    @staticmethod
    def backward(ctx, grad_aa, grad_ad, grad_da):
        ...
        # 实际实现需要完整的逆小波变换
        # 这里简化为直接上采样
        grad_aa = F.interpolate(grad_aa, scale_factor=2, mode='nearest')
        grad_ad = F.interpolate(grad_ad, scale_factor=2, mode='nearest')
        grad_da = F.interpolate(grad_da, scale_factor=2, mode='nearest')

        # 合并梯度
        grad_input = grad_aa + grad_ad + grad_da
        return grad_input, None


class DWT(nn.Module):
    def __init__(self, wavelet='sym2'):
...
class ResidualSpatialAttention(nn.Module):
    def __init__(self, in_channels):
        ...
        self.attn_gen = nn.Sequential(
            ...
            HSigmoid()  # 使用论文中的HSigmoid
        )

    def forward(self, x):
        # 残差连接
        residual = self.res_block(x)
        residual = residual + x  # 跳跃连接

        # 生成空间注意力图
        attn_map = self.attn_gen(residual)

        # 应用空间注意力
        return residual * attn_map


# 4. HSigmoid激活函数 (论文中定义)
...
class IFA(nn.Module):
    def __init__(self, in_channels):
        ...
        self.kan_att = KANChannelAttention(in_channels)
        self.rsa_att = ResidualSpatialAttention(in_channels)
...
class SpatialPyramidPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        for level in self.levels:
            # 计算池化大小
            pool_size = (h // level, w // level) if level > 1 else (h, w)
            ...
            # 展平特征
            features.append(pooled.view(bs, c * pool_size[0] * pool_size[1]))
...
class TransposedConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, scale_factor=2):
        ...
        self.conv = nn.ConvTranspose2d(
            ...
            kernel_size=3, stride=scale_factor,
...
# 8. 分支处理模块
class BranchProcessor(nn.Module):
    def __init__(self, in_channels, out_channels, dwt_levels, target_size):
        super(BranchProcessor, self).__init__()
        ...
        for _ in range(dwt_levels):
            self.dwt_layers.append(DWT())

        # 转置卷积上采样
        self.upsample = TransposedConvBlock(in_channels * 3, out_channels)

        # 注意力模块
        self.attn = IFA(out_channels)
    ...
    def forward(self, x):
        ...
        for i in range(self.dwt_levels):
            aa, ad, da = self.dwt_layers[i](x)
            # 拼接三个子带
            x = torch.cat([aa, ad, da], dim=1)
...
# 9. FMKA-Net主网络
class FMKANet(nn.Module):
    def __init__(self, num_classes=8, input_size=52):
        super(FMKANet, self).__init__()
        self.num_classes = num_classes
        self.input_size = input_size

        # 三个分支处理器
        self.branch1 = BranchProcessor(1, 64, dwt_levels=1, target_size=26)
        self.branch2 = BranchProcessor(1, 128, dwt_levels=2, target_size=13)
        self.branch3 = BranchProcessor(1, 256, dwt_levels=3, target_size=7)

        # 空间金字塔池化
        self.spp = SpatialPyramidPooling(levels=[4, 2, 1])

        # KAN分类头 (简化实现)
        self.classifier = nn.Sequential(
            nn.Linear(64 * (16 + 4 + 1) + 128 * (16 + 4 + 1) + 256 * (16 + 4 + 1), 1024),
            nn.GELU(),
    ...
    def forward(self, x):
        ...
        # 应用空间金字塔池化
        b1_spp = self.spp(b1_out)
        b2_spp = self.spp(b2_out)
        b3_spp = self.spp(b3_out)

        # 拼接所有分支的特征
        features = torch.cat([b1_spp, b2_spp, b3_spp], dim=1)
...
if __name__ == "__main__":
    # 创建模型实例
    model = FMKANet(num_classes=8, input_size=52)

    # 打印模型结构
    print(model)

    # 测试输入
    input_tensor = torch.randn(4, 1, 52, 52)  # 批大小4, 1通道, 52x52

    # 前向传播
    output = model(input_tensor)

    print(f"输入尺寸: {input_tensor.shape}")
    print(f"输出尺寸: {output.shape} (批大小 x 缺陷类别数)")
    print(f"预测分数: \n{output.data}")